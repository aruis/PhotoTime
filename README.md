# PhotoTime

PhotoTime 是一个面向摄影爱好者的 macOS 幻灯片导出工具。

## 远期目标（可迭代）
以下目标作为当前版本的长期方向，后续可根据实际进展持续微调。

1. 对标 Lightroom 的幻灯片导出能力（聚焦核心场景，不追求全产品覆盖）。
2. 极致易用、容易上手：支持拖拽导入，参数清晰，预览即所得（WYSIWYG）。
3. 导出前尽量甄别潜在风险素材，降低导出失败概率，并尽可能定位到具体问题图片。
4. 提供高价值可定制性：
   - 相框风格（相框颜色、背景颜色，含预置 + 自定义）
   - 相框边距
   - 横竖图策略（优先读取素材默认方向）
   - 展示时长、过渡时长、是否淡入淡出
   - 单轨音频导入与预览（复杂音频编辑能力后置讨论）
5. 出错不崩溃，反馈友好：即使失败也应给出可理解原因与可执行下一步。

## 设计哲学（优先级参考）
- 80% 功能完备度
- 99% 易用度
- 90% 美观度与设计感
- 80% 成功率（阶段参考值）

> 注：以上比例用于优先级取舍，不作为严格数学指标。

## 中期目标 v2（执行顺序 + 验收标准）

### 1) 导出前风险甄别闭环（Preflight）
验收标准（可打勾）：
- [ ] 导出前自动扫描所有素材（可读性、损坏、权限、尺寸/方向异常）。
- [ ] 风险按级别展示：`可继续` / `建议处理` / `必须修复`。
- [ ] 能定位到具体问题图片（至少文件名级别）。
- [ ] 用户可一键选择“跳过问题素材并继续导出”。
- [ ] Preflight 结果在导出界面可见，不依赖日志。

### 2) 预览即所得闭环（WYSIWYG）
验收标准（可打勾）：
- [ ] 预览与导出使用同一套关键参数（时长、转场、Ken Burns、边距、风格）。
- [ ] 时间轴拖动时有明确状态反馈（生成中/失败/成功）。
- [ ] 关键时刻帧一致性测试稳定通过（默认风格 + 主要变体）。
- [ ] 预览失败不影响后续继续操作（不崩溃、可恢复）。

### 3) 核心可定制性闭环
验收标准（可打勾）：
- [ ] 相框与背景支持“预置风格 + 自定义颜色”。
- [ ] 相框边距可视化可调。
- [ ] 横竖图策略可配置（默认读取素材方向，并支持覆盖）。
- [ ] 展示时长、过渡时长、淡入淡出开关在 UI 可直接调整。
- [ ] 模板导入/导出可覆盖以上配置且向后兼容。

### 4) 音频 v1 闭环（单轨）
验收标准（可打勾）：
- [ ] 支持拖拽或选择单条音频。
- [ ] 可在预览中听到音频（基础预览即可）。
- [ ] 导出视频含音频并与时间轴同步。
- [ ] 音频异常（不可读/不支持）有友好提示且不崩溃。

### 5) 最小可用流程与 UI 完成度
验收标准（可打勾）：
- [ ] 用户可在一个主界面完成“导入-预览-导出-恢复”。
- [ ] 主操作/辅助操作分区清晰，按钮状态与状态机一致。
- [ ] 失败卡片提供明确“下一步动作”，成功卡片提供“继续动作”。
- [ ] 参数校验信息就地显示（inline），无需猜测错误原因。
- [ ] 新用户在无文档条件下可完成首次导出。

## 已确认产品取舍（当前版本）
- 兼容性：最低兼容版本统一为 `macOS 14.6`。
- 优先级：按中期目标 v2 顺序推进，音频相关能力放在最后实现。
- 暂不做：项目保存/复用（前提是稳定性已足够）。
- 暂不做：面向用户的稳定性指标看板与长期跟踪（前提是导出前风险甄别充分）。
- 错误处理：重点是“不崩溃 + 友好提示 + 尽可能定位到问题图片”。
- 日志定位：日志保留给开发排查使用，但不作为用户主路径能力。
